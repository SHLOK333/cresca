module hyperperp::events {
    ...
    struct EventStore has key {
        ...
        trade_execution_events: event::EventHandle<TradeExecutionEvent>,
    }
    ...
    public struct TradeExecutionEvent has copy, drop, store {
        taker: address,
        maker: address,
        market_id: u64,
        size: u128,
    ...
    public entry fun init_events(admin: &signer) {
        ...
        move_to(admin, EventStore {
            ...
            trade_execution_events: account::new_event_handle<TradeExecutionEvent>(admin),
        });
    }
    ...
    public fun emit_trade_executed(addr: address, e: TradeExecutionEvent) acquires EventStore {
        let store = borrow_global_mut<EventStore>(addr);
        event::emit_event(&mut store.trade_execution_events, e)
    }

    public fun new_trade_execution_event(
        taker: address,
        maker: address,
        market_id: u64,
        size: u128,
